...
class PrefixListMgr(Manager):
    ...
    def _generate_dynamic_prefix_list_cmd(self, prefix_type, data, add):
        """
        Emit a raw FRR prefix-list CLI for prefix types not registered in
        PREFIX_TYPE_CONFIG. Used by msft-managed dynamic prefix lists whose
        contents are driven by config data rather than a fixed template.
        """
    ...
    def generate_prefix_list_config(self, prefix_type, data, add):
        ...
        if type_cfg is None:
            # Fall through to dynamic prefix-list handling for msft-managed types
            return self._generate_dynamic_prefix_list_cmd(prefix_type, data, add)
        ...
        # bgp_asn is required for all registered prefix types: ANCHOR_PREFIX templates use it
    ...
    def del_handler(self, key):
        ...
        if '|' in key:
            ...
            try:
                ...
                log_warn("PrefixListMgr:: Prefix '%s' format is wrong for prefix list '%s'" % (prefix_str, prefix_type))
                return True
            # For dynamic (non-registered) prefix types, recover the stored row so we can
            # emit the correct "no" CLI (which needs action/seq/ge/le from the original entry).
            if prefix_type not in PREFIX_TYPE_CONFIG:
